...
# stars if that becomes important, which I think it will)
   ...
   #    solhr_ = solhr
    ...
    # Run coronagraph 
    lam, dlam, A, q, Cratio, cp, csp, cz, cez, cD, cR, cth, DtSNR = \
        cg.count_rates(Ahr_, lamhr_, alpha, Phi, radius.value, Teff_, Rs_, semimajor.value, distance.value, exozodi.value, diameter.value, resolution.value, temperature.value, inner.value, outer.value,  solhr=solhr_, lammin=lammin, lammax=lammax, De=darkcurrent.value, Re=readnoise.value, Dtmax = dtmax.value)
   ...
   #    print lam[i], ',', Cratio[i], ',', spec[i], ',', sig[i]
    ...
    if comparison.value != lastcomparison:
      if comparison.value == 'Earth':
          fn_c = 'planets/earth_quadrature_radiance_refl.dat'
          model_c = np.loadtxt(fn_c, skiprows=8)
          lamhr_c = model_c[:,0]
          radhr_c = model_c[:,1]
          solhr_c = model_c[:,2]
    ...
    if comparison.value != 'none':
      ...
      lamC, dlamC, AC, qC, CratioC, cpC, cspC, czC, cezC, cDC, cRC, cthC, DtSNRC = \
       cg.count_rates(Ahr_c, lamhr_c, alpha, Phi, radius_c, Teff_c, Rs_c, semimajor_c, distance.value, exozodi.value, diameter.value, resolution.value, temperature.value, inner.value, outer.value,  solhr=solhr_c, lammin=lammin, lammax=lammax, De=darkcurrent.value, Re=readnoise.value, Dtmax = dtmax.value)
